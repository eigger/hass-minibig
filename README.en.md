...
- **Faithful Position Reporting**: The reported position matches the official app exactly — no assumed rounding at the endpoints. (An earlier version snapped near-end values to 0%/100%; real hardware testing showed a fully-open reading of 97% that the vendor app itself does not round, so the snapping was removed.)
...
Confirmed on real hardware (a CLWM-B06 window opener) by isolating each operation
and comparing `READ_DPS` dumps before and after:

| DP ID | Function | Status |
|---|---|---|
| `0` | Position (window opener) / power (PushMini, SwitchMini) | ✅ Confirmed — matches the official app exactly, including a non-round "fully open" reading |
| `3` | Stop (also usable for passive position polling without movement) | ✅ Confirmed |
| `10` | Battery percentage | ✅ Confirmed |

These four cover everything this integration controls or reports — day-to-day
operation needs nothing beyond them. A handful of other DP IDs exist on the tested
unit but only change during the vendor app's one-time setup wizard (mount type,
opening direction, travel-limit calibration). They are not exposed by this
integration: they're set once at installation and have no use during normal
operation, so identifying them wasn't worth the disruptive re-setup testing it would
take. See [`docs/BLE_PROTOCOL.md`](docs/BLE_PROTOCOL.md#42-modern-dp-data-point-triplet-format)
for details, or use `minibig.send_raw` below if you want to explore further yourself.
